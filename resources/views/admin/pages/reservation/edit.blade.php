...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('admin.reservation.update', $record->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="guest_id">Guest ID</label>
                <input type="text" class="form-control" id="guest_id" name="guest_id" value="{{ $record->guest_id }}"
                    required>
                <div id="guest_list"></div>
            </div>
            <div class="form-group">
                <label for="room_id">Room ID</label>
                <input type="text" class="form-control" id="room_id" name="room_id" value="{{ $record->room_id }}"
                    required>
            ...
            <div class="form-group">
                ...
                <input type="datetime-local" class="form-control" id="check_in" name="check_in"
                    value="{{ $record->check_in }}" required>
            </div>
            <div class="form-group">
                <label for="check_out">Check-Out Date</label>
                <input type="datetime-local" class="form-control" id="check_out" name="check_out"
                    value="{{ $record->check_out }}" required>
            </div>
            <div class="form-group">
                <input type="hidden" class="form-control" id="price_per_night" name="price_per_night"
                    value="{{ $record->price_per_night }}">
            </div>
            <div class="form-group">
                <label for="duration">Duration</label>
                <input type="number" class="form-control" id="duration" name="duration" value="{{ $record->duration }}"
                    required>
            </div>
            <div class="form-group">
                <label for="adults">Adults</label>
                <input type="number" class="form-control" id="adults" name="adults" value="{{ $record->adults }}"
                    required>
            </div>
            <div class="form-group">
                <label for="child">Children</label>
                <input type="number" class="form-control" id="child" name="child" value="{{ $record->children }}"
                    required>
            </div>
            <div class="form-group">
                ...
                <select class="form-control" id="status" name="status" required>
                    ...
                    <option value="1" {{ $record->status == 1 ? 'selected' : '' }}>Confirmed</option>
                    <option value="2" {{ $record->status == 2 ? 'selected' : '' }}>Rejected</option>
                    <option value="3" {{ $record->status == 3 ? 'selected' : '' }}>Cancelled</option>
                </select>
            </div>
            <div class="form-group">
